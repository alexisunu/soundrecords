...
public class SpotifyService {
    ...
    public AlbumResponse getAlbumById(String spotifyAlbumId, UUID userId) {
        ...
        Map response = WebClient.create()
                ...
                .onStatus(
                    status -> status.is4xxClientError(),
                    clientResponse -> clientResponse.bodyToMono(String.class)
                        .doOnNext(body -> log.error("Error Spotify: {}", body))
                        .then(Mono.error(new RuntimeException("Album no encontrado")))
                )
                .bodyToMono(Map.class)
                .block();

        AlbumResponse album = mapToAlbumResponse(response);

        // Agregar datos de la comunidad desde la BD
        Double avgRating = reviewRepository.findAverageRatingBySpotifyAlbumId(spotifyAlbumId);
        Integer reviewCount = reviewRepository.countBySpotifyAlbumId(spotifyAlbumId);

        album.setPlatformRating(avgRating);
        album.setPlatformReviewsCount(reviewCount != null ? reviewCount : 0);

        // Si el usuario ya reseñó este álbum, incluir su reseña
        if (userId != null) {
        reviewRepository.findByUserIdAndSpotifyAlbumId(userId, spotifyAlbumId)
                .ifPresent(review -> album.setMyReview(
                    ReviewResponse.builder()
                        .id(review.getId())
                        .userId(review.getUser().getId())
                        .username(review.getUser().getDisplayUsername())
                        .userPhotoUrl(review.getUser().getPhotoUrl())
                        .spotifyAlbumId(review.getSpotifyAlbumId())
